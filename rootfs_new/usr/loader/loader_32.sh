...
if [ -d /mnt/usb/AutoRun/LTSystemUpdate ] ; then
	...
	uImage=$(fw_printenv uImage)
	uImage=${uImage#*uImage=}
	uboot=$(fw_printenv uboot)
	uboot=${uboot#*uboot=}
...
if [ -f /mnt/usb/AutoRun/LTSystemUpdate/$uboot  ]; then	# if uImage
			
		/usr/loader/loaderPipeClient $FILEFIFO "Updating uboot image. Please wait...2bp"
 		flash_eraseall /dev/mtd1
		nandwrite -p /dev/mtd1 /mnt/usb/AutoRun/LTSystemUpdate/$uboot 
		RETCODE=$?
